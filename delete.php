...
$str = implode(',',$_POST);
if (empty($str)) {
				$hosturl = hostpath() . 'index.php?do=delete';
				appheader('-文件删除出现错误','-跳转', '3', "$hosturl");
}
appheader('-文件删除');
asort($_POST);
foreach ($_POST as $id => $filename) {
    $sy_path=mb_convert_encoding($filename,$system_coding,'UTF-8');
				if (is_file($sy_path)) {
								if (unlink($sy_path)) echo "<font color=\"blue\">删除文件 \"$filename\" 成功！</font><br>";
